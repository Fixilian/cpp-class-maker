import { isUpperLetter, isLowerLetter, isDigit, isWhitespace } from '../../Utility/Chars';
...
export class CmakeParser {
     ...
     *
     * @param commandNames list of command names to find (e.g., ["add_executable", "add_library"]).
     * @returns Array of cmake commands found in the file.
     */
    findCmakeCommands(commandNames: string[]): CmakeCommand[] {
        ...
        while (i < length) {
            ...
            results.push(new CmakeCommand(name, nameStart, argsStart, end + 1));
            i = end + 1;
        }
     ...
     *
     * @param command command descriptor object.
     * @returns array of argument strings. Empty if there are no arguments.
     */
     ...
     * @param filename file to insert (e.g. "foo.cpp").
    ...
    /**
     * Returns position for a given character offset in cmake file.
     * 
    ...
    offsetToPosition(offset: number): vscode.Position {
        const pos = getPosition(this.content, offset);
